...
describe("parseIngestionError", () => {
  test.each([
    ...
    ["DOCUMENT_MALFORMED: ignored", "DOCUMENT_MALFORMED", "This document could not be parsed.", "Re-export the file from its source application, then re-upload it."],
  ...
  test.each(["toString", "constructor", "__proto__"])("rejects inherited stored code %s", (code) => {
    expect(parseIngestionError(`${code}: hostile parser output`)).toEqual({
      code: "DOCUMENT_MALFORMED",
      message: "This document could not be parsed.",
      action: "Check Jobs and troubleshooting, then re-export the file before trying again.",
    });
...
describe("IngestionErrorDetails", () => {
  test("renders safe code, message, and action without rendering hostile stored data", () => {
    ...
    expect(html).toContain("This document could not be parsed.");
